...
public readonly record struct ContentRect
{
    ...
    public ContentRect(SKRect rect)
    {
        Value = rect;
    }

    public ContentRect(float left, float top, float right, float bottom)
        : this(new SKRect(left, top, right, bottom))
    {
    }

    public ContentSize ToSize() => new (Value.Size);
